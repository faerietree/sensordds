...
public abstract class FuzzyDemo {
    ...
    private int timeStepM = 5;  // millisec per controller update
    protected int dilation = 6;      // make time run slow
 ...
 //   private String buttonString;
    ...
    protected FuzzyDemo(InputStream fclInput,boolean applet) {
        ...
        this.applet=applet;
        //  File f=new File(fclFile);

        fclInputStream = fclInput; //this.getClass().getResourceAsStream(fclFile);
        ...
        // model timestep stays constant but we vary the rate it is called to implement dilation  
        timerM = new javax.swing.Timer(timeStepM * dilation, new ActionListener() {
        ...
        //   content.getControlPanel().setBackground(Color.GREEN);
        //   content.getControlPanel().add(new JButton("LABEL"),BorderLayout.CENTER);
    ...
    private JComponent createHtmlPanel(String url) {
        ...
        java.net.URL helpURL = FuzzyDemo.class.getResource(
                                        url);
        if (helpURL != null) {
            try {
    ...
    private void initComponents() {
        ...
        content.add(fuzzyLayout = new FuzzyDemoLayoutPanel((JPanel) view.getDisplayPanel()), BorderLayout.CENTER);
        if (!applet) fuzzyLayout.getTabPanel().add("About", createHtmlPanel("about.html"));
  
        fuzzyLayout.getTabPanel().add("Fuzzy Control Logic", createEditorPanel());

        initFisDebugPanel();

        if (!applet) fuzzyLayout.getTabPanel().add("Credits", createHtmlPanel("credits.html"));
    ...
    /**
     *  top panel has all the buttons.
     *  create generic and  delegate specific to subclasses.
     * 
     * @return
     */
    protected JPanel createEditorPanel() {
        ...
        try {
            top.add(fclPanel = new FCLPanel(fuzzyController.getFclAsInputStream()), BorderLayout.CENTER);
        } catch (IOException ex) {
            Logger.getLogger(FuzzyDemoApplet.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private void addControlsTo(JPanel cntrl) {
        ...
        cntrl.add(new JToggleButton(new AbstractAction("USE FUZZY CONTROL") {
            ...
            public void actionPerformed(ActionEvent e) {
                fuzzyControlActive = ((JToggleButton) e.getSource()).isSelected();
